...
def migrate_database():
    ...
    with app.app_context():
        ...
        try:
            ...
            # Step 1.1: Add fake_call_audio_path column to users table if it doesn't exist
            print("\n1.1. Checking users table for fake_call_audio_path...")
            try:
                connection.execute(text("SELECT fake_call_audio_path FROM users LIMIT 1"))
                print("   ✓ fake_call_audio_path column already exists")
            except Exception:
                print("   → Adding fake_call_audio_path column...")
                connection.execute(text("ALTER TABLE users ADD COLUMN fake_call_audio_path VARCHAR(255)"))
                connection.commit()
                print("   ✓ fake_call_audio_path column added")
