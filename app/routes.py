from flask import Blueprint, render_template, request, redirect, url_for, flash, session, jsonify, make_response, abort, current_app
...
from app.models import db, IncidentReport, CommunityPost, Comment, EmergencyContact, SOSAlert, UserPreference, RouteFeedback, PostLike, CommentLike
...
def _rule_based_support_reply(user_message: str):
    ...
    if any(w in msg for w in ['home','house','family','partner','husband','boyfriend']):
        tips.append("Identify a trusted neighbour or friend as a quick‑reach contact and agree on a discreet code word.")
        tips.append("Keep a small emergency kit (whistle, pepper spray where legal) near your bedroom door.")
    # Workplace safety
    if any(w in msg for w in ['work','office','boss','colleague']):
        tips.append("Document any incident with dates, times, and witnesses; report to HR or a women‑safety committee.")
        tips.append("Familiarise yourself with your company's anti‑harassment policy and the Indian Sexual Harassment of Women at Workplace (Prevention, Prohibition and Redressal) Act.")
    # Online safety
    if any(w in msg for w in ['online','instagram','whatsapp','social','dm','stalking']):
        tips.append("Enable two‑factor authentication on all accounts and regularly review privacy settings.")
        tips.append("If you receive threatening messages, screenshot them and report to the platform and local police (use 100).")
    # Night / travel safety
    if any(w in msg for w in ['night','street','bus','auto','cab','uber','ola']):
        tips.append("Share your live location with a trusted contact before starting a ride.")
        tips.append("Prefer well‑lit, busy stops and avoid isolated routes; keep your phone charged.")
    # General legal resources for Indian women
    if any(w in msg for w in ['abuse','domestic','violence','harassment']):
        tips.append("You have the right to seek protection under the Protection of Women from Domestic Violence Act, 2005.")
        tips.append("Call 181 for women’s helpline or 1091 for women’s crisis helpline for immediate assistance.")
    if not tips:
        tips.append("Take a deep breath; we can create a short safety plan together for the next 24 hours.")
    # Combine all collected tips into a single helpful message
    return " ".join(base[:2]) + " " + " ".join(tips)
...
def signup():
    ...
    if User.query.filter_by(email=email).first():
        return jsonify({'success': False, 'message': 'Account already exists, please sign in'}), 409
    ...
    session['user_name'] = getattr(new_user, 'name', '')
    session['username'] = new_user.username
    session['default_anonymous'] = new_user.default_anonymous
    session['logged_in'] = True
    return jsonify({'success': True, 'message': 'Account created and logged in', 'redirect': '/home', 'user': {'username': new_user.username, 'email': new_user.email}})
...
@bp.route('/api/upload-fake-call-audio', methods=['POST'])
def upload_fake_call_audio():
    if not session.get('logged_in'):
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    if file:
        filename = secure_filename(f"user_{session.get('user_id')}_fake_call.webm")
        upload_dir = os.path.join('app', 'uploads', 'fake_call_audio')
        ...
        user.fake_call_audio_path = f"/uploads/fake_call_audio/{filename}"
        db.session.commit()
        
        return jsonify({'success': True, 'message': 'Audio uploaded successfully', 'path': user.fake_call_audio_path})
    
    return jsonify({'success': False, 'message': 'Upload failed'}), 500

@bp.route('/uploads/fake_call_audio/<filename>')
def serve_fake_call_audio(filename):
    directory = os.path.join(os.getcwd(), 'app', 'uploads', 'fake_call_audio')
...
def _maybe_transcode_to_webm(src_path: str):
    """Transcode uploaded video to webm when possible. Returns (path, converted)."""
    if src_path.lower().endswith('.webm'):
        return src_path, False
    ffmpeg = shutil.which('ffmpeg')
    if not ffmpeg:
        print("[recordings] ffmpeg not found; keeping original format")
        return src_path, False
    base, _ = os.path.splitext(src_path)
    out_path = base + '.webm'
    try:
        # Use a pragmatic preset for quick server-side conversion
        cmd = [
            ffmpeg, '-y', '-i', src_path,
        ...
        print(f"[recordings] transcoded to webm: {out_path}")
        return out_path, True
    except Exception as e:
        print(f"[recordings] ffmpeg failed, keeping original: {e}")
        return src_path, False

def _public_base_url():
    """Return the public-facing base URL respecting proxy headers/env overrides."""
    # Highest priority: explicit env override
    env_base = os.environ.get('PUBLIC_BASE_URL')
    if env_base:
        return env_base.rstrip('/') + '/'

    # Use forwarded headers when behind a proxy/load balancer
    proto = request.headers.get('X-Forwarded-Proto', request.scheme)
    host = request.headers.get('X-Forwarded-Host') or request.host
    
    # Force relative URL to allow frontend proxy to handle domain
    return "/"

def _public_url(path_or_url: str):
    """Normalize a URL to use the public base and avoid mixed content."""
    if not path_or_url:
        return path_or_url
    # If already absolute, optionally upgrade to https to avoid mixed content
    if path_or_url.startswith('http://') or path_or_url.startswith('https://'):
        if request.scheme == 'https' and path_or_url.startswith('http://'):
            return 'https://' + path_or_url.split('://', 1)[1]
        return path_or_url
    # Relative path → join with public base
    return urljoin(_public_base_url(), path_or_url.lstrip('/'))

def _format_alert_time():
    """Format alert time in the configured timezone (default Asia/Kolkata)."""
    try:
        from zoneinfo import ZoneInfo  # py3.9+
        tz_name = os.environ.get('APP_TZ', 'Asia/Kolkata')
...
def send_sms_alert(contacts, user_name, location_link, battery_level, user_phone=None):
    ...
    if sms_style == 'otp':
        ...
        message = (
            ...
            f"Battery: {battery_text} | Time: {_format_alert_time()}"
        )
...
@bp.route('/fake-call')
def fake_call():
    """AI-powered Fake Call - React handles this route now"""
    # Check if this is an API request or direct browser request
    # If Accept header includes text/html, this might be direct navigation
    # But we want React to handle the UI, so we redirect to the React app
    # For development, React runs on port 3000, so we let the request pass through
    # This route is kept for backward compatibility when React isn't running
    basic = request.args.get('basic', '').lower() in ('1', 'true', 'yes')
    if basic:
        return render_template('fake_call.html')
    # For non-basic, let React handle it by returning index.html or redirecting
    # In production, nginx would handle this. For development, this fallback:
...
@bp.route('/api/sos-videos', methods=['GET'])
def get_sos_videos():
    """Fetch SOS alerts with video recordings for the logged-in user"""
    if not session.get('logged_in'):
        return jsonify({'success': False, 'error': 'Not logged in'}), 401
    
    user_id = session.get('user_id')
    
    # Query alerts with video recordings
    alerts = SOSAlert.query.filter(
        SOSAlert.user_id == user_id,
        SOSAlert.video_recording_url.isnot(None),
        SOSAlert.video_recording_url != ''
    ).order_by(SOSAlert.trigger_time.desc()).all()
    
    videos = []
    for alert in alerts:
        # Ensure URL is accessible (add / if missing)
        video_url = alert.video_recording_url
        if video_url and not video_url.startswith('/') and not video_url.startswith('http'):
            video_url = '/' + video_url
        video_url = _public_url(video_url)
            
        videos.append({
            'id': alert.id,
            'trigger_time': alert.trigger_time.isoformat() + 'Z',
            'video_url': video_url,
            'location': f"{alert.latitude:.4f}, {alert.longitude:.4f}" if alert.latitude and alert.longitude else "Unknown Location",
            'battery_level': alert.battery_level
        })

    # Fallback: if DB has none, surface any recorded files from recordings_log.json for this user
    if not videos:
        try:
            _, logs_dir, _ = _ensure_dirs()
            ...
            if os.path.exists(path):
                ...
                for idx, r in enumerate(reversed(data)):
                    # Filter to current user when available; otherwise include all
                    log_user = r.get('user')
                    print(f"DEBUG: Log user: {log_user}, Current user: {current_user}")
                    if current_user and log_user and str(log_user).lower() != str(current_user).lower() and log_user not in (None, ''):
                        continue
                    fname = r.get('filename')
                    if not fname:
                        continue
                    vurl = _public_url(url_for('main.serve_uploads', filename=f"sos/recordings/{fname}"))
                    videos.append({
                        'id': r.get('sosId') or f"log-{idx}",
                        'trigger_time': r.get('uploadTime') or datetime.utcnow().isoformat() + 'Z',
                        'video_url': vurl,
                        'location': 'Unknown Location',
                        'battery_level': r.get('battery')
                    })
        except Exception as e:
            print(f"sos-videos fallback error: {e}")

    return jsonify({'success': True, 'videos': videos, 'message': 'Loaded from logs' if not alerts else 'Loaded from database'})
...
# Old Flask template route - disabled in favor of React
# @bp.route('/')
# def index():
...
def serve_uploads(filename):
    ...
    response = make_response(send_from_directory(base_dir, filename, as_attachment=False))
...
def api_sos():
    ...
    try:
        ...
        if session.get('user_id'):
            ...
            if contacts:
                # Generate tracking link using public-facing base URL (respects proxy headers/env)
                tracking_link = urljoin(_public_base_url(), f"track/{sos_id}")
                user_name = user.username if user else 'User'
...
def api_sos_live():
    """Receive live location updates for an active SOS."""
    payload = request.get_json(silent=True) or {}
    sos_id = payload.get('sosId')
    lat = payload.get('latitude')
    lon = payload.get('longitude')
    
    if sos_id and lat and lon:
        # Update the main alert record with the latest location
        try:
            alert = SOSAlert.query.get(sos_id)
            if alert:
                alert.latitude = lat
                alert.longitude = lon
                db.session.commit()
        except Exception as e:
            print(f"Error updating SOS location: {e}")
            
    # In a real app, we would push this to a websocket or save to a time-series DB
    # For now, we just log it to a file
    _ensure_dirs()
    _, logs_dir, _ = _ensure_dirs()
    _append_json(os.path.join(logs_dir, 'sos_log.json'), payload)
...
def api_upload_recording():
    ...
    # If mp4 (or other) try to create a webm copy for browser-friendly playback
    final_path, converted = _maybe_transcode_to_webm(save_path)
    ...
    meta = {
        ...
        'mimetype': 'video/webm' if converted or final_filename.lower().endswith('.webm') else file.mimetype,
        'uploadTime': datetime.utcnow().isoformat(),
        'sosId': request.form.get('sosId'),
        'user': session.get('username') or 'Anonymous',
        'duration': request.form.get('duration'),
        'convertedToWebm': bool(converted)
    }
    _append_json(os.path.join(logs_dir, 'recordings_log.json'), meta)
    file_url = url_for('main.serve_uploads', filename=f"sos/recordings/{final_filename}")

    # Update SOSAlert in database
    sos_id = request.form.get('sosId')
    if sos_id:
        try:
            alert = SOSAlert.query.get(sos_id)
            if alert:
                alert.video_recording_url = file_url
                db.session.commit()
                print(f"Updated SOSAlert {sos_id} with video URL: {file_url}")
        except Exception as e:
            print(f"Error updating SOSAlert {sos_id}: {e}")
            db.session.rollback()
...
def chat_api():
    ...
    # If we reach here, Gemini API failed or key is missing
    # Return a fallback response so the frontend doesn't break
    fallback_responses = [
        "I'm here for you. Are you safe right now?",
        "Just keep walking and talking to me. I'm listening.",
        "I can hear you. Everything is going to be okay.",
        "Stay on the line with me. Where are you right now?",
    ...
    # Use chat history length to cycle through fallbacks sequentially
    # This prevents repetition when the AI is down/quota exceeded
    history_len = len(history)
    fallback_msg = fallback_responses[history_len % len(fallback_responses)]
    
    # Add to history so the count increases for next time
    history.append({'user': user_message, 'ai': fallback_msg})
    session['chat_history'] = history[-6:]
    
    # Return 200 OK with fallback message
    return jsonify({
        'success': True, 
        'message': fallback_msg, 
        'provider': 'fallback',
        'note': 'AI service unavailable, using simulation'
    })
...
def api_my_reports():
    ...
    print(f"DEBUG: Session in my_reports: {dict(session)}")
    ...
    return jsonify({
        ...
        'reports': [{
            'id': r.id,
            'title': f"{r.incident_type.replace('_', ' ').title()} - {r.who_involved}",
...
def api_get_report(report_id):
    ...
    return jsonify({
        ...
        'report': {
            'id': report.id,
            'title': f"{report.incident_type.replace('_', ' ').title()} - {report.who_involved}",
...
@bp.route('/api/submit_report', methods=['POST'])
def api_submit_report():
    ...
    try:
        ...
        title = request.form.get('who_involved', 'Incident Report')
        incident_type = request.form.get('incident_type', 'other')
        location = request.form.get('location', '')
        description = request.form.get('impact', '')
        when_occurred = request.form.get('when_occurred')
        ...
        if when_occurred:
            try:
                dt = datetime.fromisoformat(when_occurred)
        ...
        report = IncidentReport(
            user_id=user_id,
            who_involved=title, # 'title' variable holds 'who_involved' from form
            incident_type=incident_type,
            location=location,
            impact=description, # 'description' variable holds 'impact' from form
            additional_details=request.form.get('additionalInfo', ''),
        ...
        if 'files' in request.files:
            ...
            # Logic to save files would go here
            pass
        ...
        return jsonify({'success': False, 'error': str(e)}), 500
@bp.route('/api/reports/<int:report_id>/generate_legal', methods=['POST'])
def generate_legal_report(report_id):
    """Generate a formal legal report using Gemini AI"""
    if not session.get('logged_in'):
        return jsonify({'success': False, 'error': 'Not logged in'}), 401
    ...
    try:
        # Gather all available report details
        incident_type = report.incident_type or "Incident"
        incident_date = str(report.incident_date) if report.incident_date else "Date not specified"
        incident_time = str(report.incident_time) if report.incident_time else "Time not specified"
        location = report.location or "Location not specified"
        who_involved = report.who_involved or "Not specified"
        impact = report.impact or "No impact details provided"
        additional_details = report.additional_details or ""
...
- Location: {location}
- Parties Involved: {who_involved}
- Primary Impact: {impact}
"""
        
        if additional_details:
            all_details += f"- Additional Information: {additional_details}\n"
            
        if report.first_time:
            all_details += f"- First Occurrence: {report.first_time}\n"
        if report.frequency:
            all_details += f"- Frequency: {report.frequency}\n"
        
        prompt = f"""
You are a legal documentation expert specializing in Indian criminal law. Create a comprehensive LEGAL AFFIDAVIT / COMPLAINT REPORT based on the following incident details. This document will be used for filing with police authorities under Indian Penal Code (IPC) / Bharatiya Nyaya Sanhita (BNS).

{all_details}

IMPORTANT INSTRUCTIONS:
1. Write in formal legal language suitable for Indian courts and police
2. Expand the incident description into a detailed narrative (minimum 200 words)
3. Include proper legal sections and terminology
4. Structure as a formal affidavit/sworn statement
5. Do NOT use template placeholders - use the actual information provided
6. Add relevant IPC/BNS sections based on the incident type
7. Make it comprehensive and court-ready

Generate a document with the following structure:

═══════════════════════════════════════════════════════════
              INCIDENT COMPLAINT AFFIDAVIT
         (Under Section 154, Code of Criminal Procedure, 1973)
═══════════════════════════════════════════════════════════

BEFORE THE STATION HOUSE OFFICER
[Police Station Name/Jurisdiction]

COMPLAINT UNDER SECTION 154 Cr.P.C.

COMPLAINANT DETAILS:
Name: {"Anonymous Complainant" if report.is_anonymous else "[Complainant Name]"}
Status: {"Anonymous Report" if report.is_anonymous else "On Record"}
...
1. INCIDENT CLASSIFICATION
   Nature of Offense: {incident_type.upper().replace('_', ' ')}
...
3. DETAILED DESCRIPTION OF THE INCIDENT
   [Expand this into a comprehensive narrative of at least 200 words describing:
   - What exactly happened
   - The sequence of events
   - The accused person's actions
   - The impact on the complainant
   - Any witnesses or evidence
   - Exact words/actions if relevant
   
   Use the following information as the basis:
   - Involved Party/Accused: {who_involved}
   - Impact on Complainant: {impact}
   - Additional Context: {additional_details if additional_details else "As described above"}
   ]

4. ACCUSED DETAILS
   Identity/Description: {who_involved}
   {"Relationship to Complainant: [If applicable based on who_involved field]" if who_involved else ""}

5. IMPACT AND CONSEQUENCES
   {impact}
   {"This incident has had the following specific effects: [Expand on psychological, physical, social impacts]" if impact else ""}

6. WITNESS INFORMATION
   [State if witnesses were present based on available information]

7. EVIDENCE AVAILABLE
   [Describe any evidence mentioned or implied in the report]

8. PREVIOUS INCIDENTS
   {"This is " + ("the first such incident" if report.first_time == "yes" else "a repeated occurrence") if report.first_time else "First reported incident"}
   {f"Frequency: {report.frequency}" if report.frequency else ""}

═══════════════════════════════════════════════════════════
                  LEGAL GROUNDS AND PRAYER
═══════════════════════════════════════════════════════════

APPLICABLE LEGAL PROVISIONS:
Based on the nature of the incident ({incident_type}), the following sections appear applicable:
[List relevant IPC/BNS sections - example:
- Section 323 IPC (Voluntarily causing hurt) / Section 115 BNS
- Section 506 IPC (Criminal intimidation) / Section 351 BNS
- Section 354 IPC (Assault or criminal force to woman with intent to outrage her modesty) / Section 74 BNS
etc. - Choose appropriate sections based on the incident type]

PRAYER:
I, the complainant, respectfully request the Hon'ble Station House Officer to:
1. Register this complaint as a First Information Report (FIR) under the relevant sections of law
2. Conduct thorough investigation into the matter
3. Take appropriate legal action against the accused
4. Ensure protection of the complainant from any further harassment or intimidation
5. Keep the complainant's identity {"confidential as requested" if report.is_anonymous else "on record"}

VERIFICATION:
I, the complainant, do hereby solemnly affirm and declare that the contents stated above are true and correct to the best of my knowledge and belief, and nothing material has been concealed therefrom.

Place: [Location]
Date: {datetime.now().strftime('%d %B %Y')}

Signature: _______________
{"(Anonymous - Identity Protected)" if report.is_anonymous else "(Complainant)"}
...
[This section to be filled by police authorities]

═══════════════════════════════════════════════════════════

Note: This is a computer-generated legal document draft based on information provided by the complainant. It should be reviewed and verified before submission to authorities.
        """
        ...
        print(f"🔍 Calling Gemini API at: {url[:50]}...")
        response = requests.post(url, headers={'Content-Type': 'application/json'}, json=payload, timeout=30)
        
        print(f"📡 Response Status: {response.status_code}")
        if response.status_code != 200:
            print(f"❌ API Error Response: {response.text[:500]}")
            return jsonify({'success': False, 'error': f'AI generation failed: {response.status_code}'}), 500
            
        data = response.json()
        print(f"✅ API Response received, extracting text...")
        legal_text = data['candidates'][0]['content']['parts'][0]['text']
        
        # Save to database
        report.ai_summary = legal_text
        db.session.commit()
        print(f"💾 Legal report saved to database")
        
        return jsonify({
            'success': True,
            'legal_report': legal_text
        })
...
@bp.route('/api/reports/<int:report_id>/download_legal', methods=['GET'])
def download_legal_report(report_id):
    """Download the generated legal report as a text file"""
    if not session.get('logged_in'):
        return jsonify({'success': False, 'error': 'Not logged in'}), 401
    ...
    if not report or not report.ai_summary:
        return jsonify({'success': False, 'error': 'Legal report not found'}), 404
        
    # Create response with text content
    response = make_response(report.ai_summary)
    response.headers['Content-Type'] = 'text/plain'
    response.headers['Content-Disposition'] = f'attachment; filename=Legal_Statement_Report_{report.id}.txt'
...
# ============================================================================
# COMMUNITY FEED & AI MODERATION ENDPOINTS
# ============================================================================

# Offensive words list for moderation
OFFENSIVE_WORDS = [
    # Severe profanity only
    'fuck', 'fucking', 'fucked', 'fucker', 'fck', 'f*ck', 'f**k',
    'shit', 'bullshit', 'shitty',
    # Gender-based slurs (women)
    'bitch', 'whore', 'slut', 'hoe', 'thot', 'skank', 'cunt',
    # Misogynistic terms
    'belong in kitchen', 'dishwasher', 'sandwich maker',
    # Gender-based slurs (men)
    'bastard', 'dick', 'prick', 'asshole',
    # Derogatory disability terms
    'retard', 'retarded',
    # Racist/discriminatory
    'nigger', 'nigga', 'chink', 'spic',
    # Threatening violence
    'kill you', 'murder you', 'death threat',
    # Sexual harassment
    'rape', 'molest', 'grope',
    # Suicide-related
...
def moderate_content_with_ai(content):
    """Use Gemini AI to moderate content for offensive language"""
    # First do quick basic filter
    basic_result = basic_word_filter(content)
    if basic_result['status'] == 'rejected':
        return basic_result
    
    try:
        # Build prompt for Gemini
        prompt = f"""
You are a strict content moderation AI for a women's safety platform. Analyze the following user-generated content for:
1. Any profanity or swear words
2. Gender-based offensive language (sexist, misogynistic, hateful)
3. Abusive, threatening, or harassing language
4. Inappropriate sexual content
5. Bullying or harmful content

Content to analyze:
\"\"\"{content}\"\"\"

BE STRICT. If there is ANY questionable content, reject it.

Respond ONLY with a JSON object in this exact format:
{{
  "status": "approved" or "rejected",
  "flags": ["word1", "word2"] or [],
  "reason": "Brief explanation if rejected"
}}

Rules:
- "approved": Content is completely clean and safe for a support community
- "rejected": Contains ANY profanity, offensive language, or harmful content
"""
        ...
        if response.status_code == 200:
            ...
            print(f"❌ AI moderation API error: {response.status_code}")
            return basic_result
...
def basic_word_filter(content):
    """Fallback basic word filtering - STRICT"""
    ...
    # STRICT: Reject if ANY offensive word is found
    if len(flagged_words) > 0:
        print(f"🚫 Content rejected - flagged words: {flagged_words}")
        return {'status': 'rejected', 'flags': flagged_words, 'reason': f'Contains offensive content: {", ".join(flagged_words)}'}
    else:
        return {'status': 'approved', 'flags': [], 'reason': 'Content appears clean'}
...
def get_community_posts():
    ...
    try:
        ...
        posts = CommunityPost.query.filter_by(moderation_status='approved').order_by(
...
@bp.route('/api/community/posts', methods=['POST'])
def create_community_post():
    """Create a new community post with AI moderation"""
    if not session.get('logged_in'):
        return jsonify({'success': False, 'error': 'Please log in to post'}), 401
    ...
    try:
        ...
        # AI Moderation
        moderation_result = moderate_content_with_ai(content)
        ...
        post = CommunityPost(
            ...
            moderation_flags=json.dumps(moderation_result.get('flags', []))
        )
        ...
        return jsonify({
            'success': True,
            'post': post.to_dict(session.get('user_id')),
            'moderation': moderation_result
        }), 201
...
@bp.route('/api/community/posts/<int:post_id>/like', methods=['POST'])
...
@bp.route('/api/community/posts/<int:post_id>/share', methods=['POST'])
...
# Comments Endpoints
@bp.route('/api/community/posts/<int:post_id>/comments', methods=['GET'])
def get_comments(post_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'comments': [comment.to_dict(current_user_id) for comment in comments]
        })
...
def add_community_comment(post_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'comment': comment.to_dict(session.get('user_id')),
            'moderation': moderation_result
        }), 201
...
@bp.route('/api/community/comments/<int:comment_id>/like', methods=['POST'])
