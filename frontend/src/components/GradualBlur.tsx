...
function GradualBlur(props: GradualBlurProps) {
  ...
  const containerStyle = useMemo(() => {
    ...
    if (isVertical) {
      ...
      // Apply bottomOffset when position is 'bottom' and target is 'page'
      if (config.position === 'bottom' && isPageTarget && config.bottomOffset && config.bottomOffset !== '0px') {
        (baseStyle as any)[config.position] = config.bottomOffset;
      } else {
